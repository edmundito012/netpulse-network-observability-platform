...
class CorrelationAnalyticsService:
    ...
    def get_summary(
        ...
        *,
        ...
        totals = CorrelationAnalyticsRepository.get_totals(
            db=db,
            window_started_at=window_started_at,
        )

        total_evaluations = int(totals["total_evaluations"])

        applied_decisions = int(totals["applied_decisions"])

        failed_decisions = int(totals["failed_decisions"])

        pending_decisions = int(totals["pending_decisions"])

        incidents_created = int(totals["incidents_created"])

        existing_incidents_matched = int(totals["existing_incidents_matched"])

        successful_decisions = applied_decisions

        application_success_rate = cls._percentage(
            successful_decisions,
            (successful_decisions + failed_decisions),
        )

        incident_reuse_rate = cls._percentage(
            existing_incidents_matched,
            (existing_incidents_matched + incidents_created),
        )

        recent = CorrelationAnalyticsRepository.get_recent(
            db=db,
            window_started_at=window_started_at,
            limit=recent_limit,
        )
        ...
        return CorrelationAnalyticsSummary(
            ...
            existing_incidents_matched=(existing_incidents_matched),
            no_action_decisions=int(totals["no_action_decisions"]),
            ...
            incident_reuse_rate=(incident_reuse_rate),
            estimated_incidents_avoided=(existing_incidents_matched),
            outcomes=cls._get_counts(
                db=db,
                column=(IncidentCorrelation.outcome),
                window_started_at=(window_started_at),
            ),
            application_statuses=cls._get_counts(
                db=db,
                column=(IncidentCorrelation.application_status),
                window_started_at=(window_started_at),
            ),
            signal_families=cls._get_counts(
                db=db,
                column=(IncidentCorrelation.signal_family),
                window_started_at=(window_started_at),
            ),
            recent_correlations=[
                cls._build_recent_item(correlation) for correlation in recent
            ],
        )
    ...
    def _get_counts(
        ...
        *,
        ...
        rows = CorrelationAnalyticsRepository.count_by_column(
            db=db,
            column=column,
            window_started_at=(window_started_at),
        )
    ...
    def _build_recent_item(
        ...
        return CorrelationAnalyticsRecentItem(
            ...
            application_status=(correlation.application_status.value),
            signal_family=(correlation.signal_family.value),
            score=float(correlation.score),
            evaluated_at=(correlation.evaluated_at),
        )
