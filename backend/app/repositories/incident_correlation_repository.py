...
class IncidentCorrelationRepository:
    ...
    def get_by_id(
        ...
        statement = (
            ...
            .where(IncidentCorrelation.id == correlation_id)
            .options(
                joinedload(IncidentCorrelation.source_alert),
                joinedload(IncidentCorrelation.target_incident),
            )
        )
    ...
    def get_by_key(
        ...
        statement = (
            select(IncidentCorrelation)
            .where(IncidentCorrelation.correlation_key == correlation_key)
            .options(
                joinedload(IncidentCorrelation.source_alert),
                joinedload(IncidentCorrelation.target_incident),
            )
    ...
    def get_latest_for_alert(
        ...
        *,
        ...
        statement = (
            select(IncidentCorrelation)
            .where(IncidentCorrelation.source_alert_id == source_alert_id)
            .options(
                joinedload(IncidentCorrelation.source_alert),
                joinedload(IncidentCorrelation.target_incident),
            )
    ...
    def find_candidates(
        ...
        *,
        ...
        observed_at = (
            source_alert.last_seen_at or source_alert.created_at or datetime.now(UTC)
        )
        ...
        candidate_ids_statement = (
            ...
            .where(
                Incident.status.in_(cls.ACTIVE_INCIDENT_STATUSES),
                Alert.device_id == source_alert.device_id,
            ...
            .order_by(
                func.max(Alert.last_seen_at).desc(),
                IncidentAlert.incident_id.desc(),
            )
            .limit(limit)
        )

        candidate_ids = list(db.scalars(candidate_ids_statement).all())
        ...
        statement = (
            ...
            .options(selectinload(Incident.alert_links).joinedload(IncidentAlert.alert))
        )

        incidents = list(db.scalars(statement).unique().all())

        incident_order = {
            incident_id: position for position, incident_id in enumerate(candidate_ids)
        }

        incidents.sort(key=lambda incident: incident_order[incident.id])
    ...
    def get_paginated(
        ...
        *,
        ...
        if source_alert_id is not None:
            filters.append(IncidentCorrelation.source_alert_id == source_alert_id)

        if target_incident_id is not None:
            filters.append(IncidentCorrelation.target_incident_id == target_incident_id)

        if outcome is not None:
            filters.append(IncidentCorrelation.outcome == outcome)

        if application_status is not None:
            filters.append(IncidentCorrelation.application_status == application_status)

        if signal_family is not None:
            filters.append(IncidentCorrelation.signal_family == signal_family)

        count_statement = select(func.count(IncidentCorrelation.id)).where(*filters)
    ...
    def mark_applied(
        ...
        *,
        ...
        metadata.update(
            {
                "applied_incident_id": (applied_incident_id),
                "applied_incident_public_id": (applied_incident_public_id),
                "incident_created": (incident_created),
                "alert_attached": (alert_attached),
            }
        )

        correlation.application_status = CorrelationApplicationStatus.APPLIED

        correlation.applied_at = applied_at or datetime.now(UTC)
    ...
    def mark_failed(
        ...
        *,
        ...
        metadata["application_failure"] = normalized_reason

        correlation.application_status = CorrelationApplicationStatus.FAILED

        correlation.failure_reason = normalized_reason
    ...
    def build_create_command(
        *,
        ...
        return IncidentCorrelationCreate(
            ...
            outcome=outcome,
            application_status=(CorrelationApplicationStatus.EVALUATED),
            signal_family=signal_family,
            score=Decimal(f"{score:.4f}"),
            threshold=Decimal(f"{threshold:.4f}"),
            reasons=reasons,
