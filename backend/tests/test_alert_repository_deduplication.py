...
def create_device() -> int:
    ...
    device = Device(
        ...
        ip_address=(f"10.71.{int(unique_id[:2], 16)}.{int(unique_id[2:4], 16)}"),
...
def test_register_occurrence_updates_existing_alert() -> None:
    ...
    alert = AlertRepository.create(
        ...
        deduplication_key=(f"device:{device_id}:packet_loss"),
    ...
    assert updated_alert.message == "Critical packet loss"
    assert updated_alert.last_seen_at >= original_last_seen_at
