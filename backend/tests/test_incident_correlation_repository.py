...
def create_alert(
    ...
    *,
    ...
    alert = Alert(
        ...
        deduplication_key=(f"correlation-test:{device_id}:{uuid4().hex}"),
...
def test_create_and_get_by_key() -> None:
    ...
    try:
        ...
        correlation_key = f"test-correlation:{uuid4().hex}"
        ...
        command = IncidentCorrelationRepository.build_create_command(
            ...
            score=0.0,
            threshold=0.65,
            reasons=[CorrelationReason.NO_CANDIDATE_INCIDENT],
            candidate_count=0,
            window_seconds=900,
            explanation=("No incident candidate was found."),
            metadata={
                "test": True,
            },
        )

        created = IncidentCorrelationRepository.create(
            db=db,
            command=command,
        )

        result = IncidentCorrelationRepository.get_by_key(
            db=db,
            correlation_key=correlation_key,
        )
        ...
        assert result.reasons == [CorrelationReason.NO_CANDIDATE_INCIDENT.value]
...
def test_get_or_create_is_idempotent() -> None:
    ...
    try:
        ...
        command = IncidentCorrelationRepository.build_create_command(
            correlation_key=(f"idempotent:{uuid4().hex}"),
            source_alert_id=alert.id,
            target_incident_id=None,
            outcome=(CorrelationOutcome.CREATE_NEW),
            signal_family=(CorrelationSignalFamily.PERFORMANCE),
            score=0.25,
            threshold=0.65,
            reasons=[CorrelationReason.SCORE_BELOW_THRESHOLD],
            candidate_count=1,
            window_seconds=900,
            explanation=("Candidate score was below threshold."),
        )

        first, first_created = IncidentCorrelationRepository.get_or_create(
            db=db,
            command=command,
        )

        second, second_created = IncidentCorrelationRepository.get_or_create(
            db=db,
            command=command,
        )
...
def test_find_candidates_returns_same_device_incident() -> None:
    ...
    try:
        ...
        assert any(candidate.id == incident.id for candidate in candidates)
    finally:
        db.close()
...
def test_paginated_correlations_support_filters() -> None:
    ...
    try:
        ...
        command = IncidentCorrelationRepository.build_create_command(
            correlation_key=(f"pagination:{uuid4().hex}"),
            source_alert_id=alert.id,
            target_incident_id=None,
            outcome=(CorrelationOutcome.CREATE_NEW),
            signal_family=(CorrelationSignalFamily.STABILITY),
            score=0.0,
            threshold=0.65,
            reasons=[CorrelationReason.NO_CANDIDATE_INCIDENT],
            candidate_count=0,
            window_seconds=900,
            explanation=("No candidate incident was found."),
        )
        ...
        result = IncidentCorrelationRepository.get_paginated(
            db=db,
            source_alert_id=alert.id,
            outcome=(CorrelationOutcome.CREATE_NEW),
            page=1,
