...
def test_incident_portfolio_page_is_available() -> None:
    response = client.get("/portfolio/incidents")

    assert response.status_code == 200
    assert "Incident Operations" in response.text
    assert 'data-dashboard-ready="false"' in response.text
    assert "/portfolio/incidents/data" in response.text


@patch("app.api.portfolio_incidents.IncidentPortfolioService.get_summary")
def test_incident_portfolio_data_endpoint(
    summary_mock,
) -> None:
    summary_mock.return_value = IncidentPortfolioSummary(
        total_incidents=5,
        active_incidents=2,
    ...
    response = client.get("/portfolio/incidents/data")
    ...
    assert payload["latest_incidents"][0]["public_id"] == "INC-2026-000005"
