...
def test_user_actor_context() -> None:
    ...
    assert actor.actor_type == IncidentTimelineActorType.USER
...
def test_system_actor_context() -> None:
    ...
    assert actor.actor_type == IncidentTimelineActorType.SYSTEM
    assert actor.actor_id is None

    assert actor.actor_label == ("NetPulse Incident Engine")
...
def test_automation_actor_context() -> None:
    ...
    assert actor.actor_type == IncidentTimelineActorType.AUTOMATION
    assert actor.actor_id is None

    assert actor.actor_label == ("NetPulse Correlation Engine")
