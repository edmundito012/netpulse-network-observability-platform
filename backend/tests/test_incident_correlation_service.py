...
def create_alert(
    ...
    *,
    ...
    alert = Alert(
        ...
        deduplication_key=(f"correlation-service:{device_id}:{uuid4().hex}"),
        severity=severity,
...
def test_evaluate_matches_strong_candidate() -> None:
    ...
    try:
        ...
        result = IncidentCorrelationService.evaluate(
            db=db,
        ...
        assert result.target_incident_id == incident.id
...
def test_evaluate_recommends_new_incident_without_candidates() -> None:
    ...
    try:
        ...
        assert CorrelationReason.NO_CANDIDATE_INCIDENT in result.reasons
    finally:
        db.close()
...
def test_evaluate_and_persist_is_idempotent() -> None:
    ...
    try:
        ...
        assert second_correlation.id == first_correlation.id
...
def test_configuration_change_creates_new_evaluation() -> None:
    ...
    try:
        ...
        _, first, first_created = IncidentCorrelationService.evaluate_and_persist(
            db=db,
            source_alert_id=source_alert.id,
            configuration=(
                CorrelationConfiguration(
        ...
        _, second, second_created = IncidentCorrelationService.evaluate_and_persist(
            db=db,
            source_alert_id=source_alert.id,
            configuration=(
                CorrelationConfiguration(
...
def test_correlation_key_is_deterministic() -> None:
    ...
    first = IncidentCorrelationService.build_correlation_key(
        source_alert_id=50,
        configuration=configuration,
    )

    second = IncidentCorrelationService.build_correlation_key(
        source_alert_id=50,
        configuration=configuration,
    )
    ...
    assert first.startswith("correlation:v1:alert:50:")
